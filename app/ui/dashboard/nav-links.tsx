...
import { HomeIcon, Cog6ToothIcon, PowerIcon, DocumentTextIcon, Squares2X2Icon } from "@heroicons/react/24/outline";
...
const settingsLinks = [
  ...
  { name: "Logout", href: "#", icon: PowerIcon, onClick: async () => await signOut({ callbackUrl: "/" }) },
];
      ...
      <nav className="space-y-2 px-4 mt-10">
        {links.map((link) => {
          ...
          return (
            ...
            >
              {isActive && <div className="absolute left-0 top-0 h-full w-1 bg-white rounded-r-full" />}
